...
class User(mongoengine.EmbeddedDocument):
    email = mongoengine.StringField(max_length=255, unique=True, required=True)
    full_name = mongoengine.StringField(max_length=70, required=True)
    bonus_card_number = mongoengine.StringField(min_length=10, max_length=20, unique=True, required=True)
...
class BaseDocument(mongoengine.Document):
    ...
    def save(self):
        ...
        if hasattr(self, 'created_on') and not self.created_on:
            ...
            ) if hasattr(g, 'user_info') else User(username="default", user_id="0")
